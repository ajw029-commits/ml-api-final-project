...
def load_artifacts():
    ...
    return (
        ret_pess_model,
        joblib.load(BASE_DIR / "loan_status_model.pkl"),
        joblib.load(BASE_DIR / "classifier_scaler.pkl"),
    )


def build_feature_frame(inputs: dict) -> pd.DataFrame:
    earliest_credit_line = pd.to_datetime(inputs["earliest_cr_line"])
    credit_age = REFERENCE_YEAR - earliest_credit_line.year
    fico_avg = (inputs["fico_range_high"] + inputs["fico_range_low"]) / 2.0
    ...
    row.update(
        {
            ...
            "annual_inc": float(np.log1p(inputs["annual_inc"])),
            "dti": DTI_MEDIAN if inputs["dti"] == 999 else inputs["dti"],
            "delinq_2yrs": inputs["delinq_2yrs"],
            "open_acc": inputs["open_acc"],
            "pub_rec": inputs["pub_rec"],
            "revol_bal": min(inputs["revol_bal"], REVOL_BAL_CAP),
            "revol_util": min(inputs["revol_util"], REVOL_UTIL_CAP),
            "term_num": inputs["term_num"],
...
def predict_outcomes(inputs: dict) -> tuple[float, float]:
    ret_pess_model, loan_status_model, classifier_scaler = load_artifacts()
    ...
    return default_probability, pessimistic_return
...
def inject_theme():
    st.markdown(
        ...
        <style>
            .stApp {{
                background:
                    linear-gradient(180deg, {BUCKNELL_PAPER} 0%, #ffffff 38%, #ffffff 100%);
                color: {BUCKNELL_INK};
            }}
            .stApp [data-testid="stForm"] {{
                ...
                box-shadow: 0 18px 40px rgba(0, 34, 68, 0.08);
            }}
            .stApp [data-testid="stMetric"] {{
                background: linear-gradient(180deg, #ffffff 0%, #f8fafc 100%);
                border: 1px solid rgba(0, 56, 101, 0.14);
                border-top: 4px solid {BUCKNELL_ORANGE};
                border-radius: 8px;
                padding: 0.9rem 1rem;
            }}
            ...
            .stApp .stForm button[type="submit"],
            .stApp div[data-testid="stFormSubmitButton"] > button {{
                background: {BUCKNELL_ORANGE} !important;
                border: 1px solid {BUCKNELL_ORANGE} !important;
                color: #ffffff !important;
                font-weight: 700 !important;
                box-shadow: 0 10px 24px rgba(232, 119, 34, 0.22) !important;
            }}
            .stApp .stButton > button:hover,
            .stApp .stForm button[type="submit"]:hover,
            .stApp div[data-testid="stFormSubmitButton"] > button:hover {{
                background: #cf641c !important;
                border-color: #cf641c !important;
                color: #ffffff !important;
            }}
            .stApp .stButton > button:focus,
            .stApp .stForm button[type="submit"]:focus,
            .stApp div[data-testid="stFormSubmitButton"] > button:focus {{
                background: {BUCKNELL_ORANGE} !important;
                border-color: {BUCKNELL_BLUE} !important;
                box-shadow: 0 0 0 0.2rem rgba(0, 56, 101, 0.18) !important;
                color: #ffffff !important;
            }}
            .stApp label, .stApp .stMarkdown, .stApp .stCaption {{
                color: {BUCKNELL_INK};
            }}
            .bucknell-hero {{
                background:
                    linear-gradient(135deg, rgba(0, 56, 101, 0.98), rgba(0, 56, 101, 0.88)),
                    linear-gradient(90deg, rgba(232, 119, 34, 0.2), rgba(232, 119, 34, 0));
                border-radius: 8px;
            ...
            .bucknell-hero::after {{
                ...
                width: 10rem;
                height: 10rem;
                background: radial-gradient(circle, rgba(232, 119, 34, 0.26) 0%, rgba(232, 119, 34, 0) 70%);
            }}
            .bucknell-badge {{
                display: inline-flex;
                align-items: center;
...
def render_header():
    st.markdown(
        f"""
        <section class="bucknell-hero">
            <div class="bucknell-badge">
                <div class="bucknell-logo">{BUCKNELL_LOGO_SVG}</div>
            </div>
            <div class="bucknell-kicker">Bucknell University Loan Analytics</div>
            <h1>Loan Outcome Predictor</h1>
            <p>
                Enter borrower and loan details to estimate default risk and expected
                pessimistic return for a prospective lending decision.
            </p>
        </section>
        """,
        unsafe_allow_html=True,
...
def render_app():
    st.set_page_config(page_title="Loan Outcome Predictor", page_icon=":bank:", layout="wide")
    inject_theme()
    render_header()
    ...
    with st.form("loan_application"):
        ...
        with col1:
            ...
            int_rate = st.number_input("Interest rate (%)", min_value=0.0, max_value=40.0, value=12.5, step=0.1)
            annual_inc = st.number_input("Annual income ($)", min_value=0.0, value=85000.0, step=1000.0)
            dti = st.number_input("Debt-to-income ratio", min_value=0.0, max_value=999.0, value=17.73, step=0.1)

        with col2:
            delinq_2yrs = st.number_input("Delinquencies in last 2 years", min_value=0, max_value=50, value=0, step=1)
            open_acc = st.number_input("Open credit lines", min_value=0, max_value=100, value=8, step=1)
            pub_rec = st.number_input("Public derogatory records", min_value=0, max_value=20, value=0, step=1)
            revol_bal = st.number_input("Revolving balance ($)", min_value=0.0, value=12000.0, step=500.0)

        with col3:
            revol_util = st.number_input("Revolving utilization (%)", min_value=0.0, max_value=200.0, value=45.0, step=0.5)
            term_num = st.selectbox("Loan term (months)", options=[36, 60], index=0)
            earliest_cr_line = st.date_input("Earliest credit line", value=pd.Timestamp("2008-01-01"))
            fico_range_low = st.number_input("FICO range low", min_value=300, max_value=850, value=690, step=1)
            fico_range_high = st.number_input("FICO range high", min_value=300, max_value=850, value=710, step=1)

        st.subheader("Borrower profile")
        col4, col5, col6, col7 = st.columns(4)

        with col4:
            home_ownership = st.selectbox("Home ownership", options=HOME_OWNERSHIP_OPTIONS, index=1)
        with col5:
            verification_status = st.selectbox("Verification status", options=VERIFICATION_OPTIONS, index=0)
        with col6:
            purpose = st.selectbox("Loan purpose", options=PURPOSE_OPTIONS, index=2)
        with col7:
            emp_length = st.selectbox("Employment length", options=EMPLOYMENT_OPTIONS, index=0)

        submitted = st.form_submit_button("Estimate outcome", use_container_width=True)
    ...
    if submitted:
        user_inputs = {
            ...
            "earliest_cr_line": str(earliest_cr_line),
        ...
        if user_inputs["fico_range_high"] < user_inputs["fico_range_low"]:
            st.error("FICO range high must be greater than or equal to FICO range low.")
            return

        default_probability, pessimistic_return = predict_outcomes(user_inputs)
        fully_paid_probability = 1.0 - default_probability
        recommendation, recommendation_state = recommend_action(
            fully_paid_probability,
            pessimistic_return,
        )

        metric_col1, metric_col2, metric_col3 = st.columns(3)
        metric_col1.metric("Recommended course of action", recommendation)
        metric_col2.metric("Fully paid likelihood", f"{fully_paid_probability:.1%}")
